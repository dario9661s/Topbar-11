...
function install() {
  ...
  async function fetchScriptTags() {
    const { data } = await axios.get(
      `https://strange-chicken-26.loca.lt/script_tag/all`
    );
  ...
  async function handleAction() {
    if (!isInstalled) {
      axios.post(`https://strange-chicken-26.loca.lt/script_tag`);
    } else {
      axios.delete(
        `https://strange-chicken-26.loca.lt/script_tag/?id=${scriptTagId}`
      );
    }
