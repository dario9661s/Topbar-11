...
function install() {
  ...
  async function fetchScriptTags() {
    const { data } = await axios.get(
      `https://cowardly-ladybug-60.loca.lt/script_tag/all`
    );
  ...
  async function handleAction() {
    if (!isInstalled) {
      axios.post(`https://cowardly-ladybug-60.loca.lt/script_tag`);
    } else {
      axios.delete(
        `https://cowardly-ladybug-60.loca.lt/script_tag/?id=${scriptTagId}`
      );
    }
