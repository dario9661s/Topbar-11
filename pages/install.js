...
function install() {
  ...
  async function fetchScriptTags() {
    const { data } = await axios.get(
      `https://afraid-crab-100.loca.lt/script_tag/all`
    );
  ...
  async function handleAction() {
    if (!isInstalled) {
      axios.post(`https://afraid-crab-100.loca.lt/script_tag`);
    } else {
      axios.delete(`https://afraid-crab-100.loca.lt/script_tag/?id=${scriptTagId}`);
    }
