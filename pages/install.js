...
function install() {
  ...
  async function fetchScriptTags() {
    const { data } = await axios.get(
      `https://mighty-hound-83.loca.lt/script_tag/all`
    );
  ...
  async function handleAction() {
    if (!isInstalled) {
      axios.post(`https://mighty-hound-83.loca.lt/script_tag`);
    } else {
      axios.delete(
        `https://mighty-hound-83.loca.lt/script_tag/?id=${scriptTagId}`
      );
    }
